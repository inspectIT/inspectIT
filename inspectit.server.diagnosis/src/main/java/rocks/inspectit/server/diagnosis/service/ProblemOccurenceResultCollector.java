...
public class ProblemOccurenceResultCollector implements ISessionResultCollector<InvocationSequenceData, List<ProblemOccurrence>> {
	...
	public List<ProblemOccurrence> collect(SessionContext<InvocationSequenceData> sessionContext) {
		...
		for (Tag leafTag : leafTags) {
			...
			// create RootCause object as we do not want to add AggregatedDiagnosisInvocationData to
			// the ProblemOccurence. RootCause is a simpler representation of it.
			RootCause rootCause = new RootCause(rootCauseInvocations.getMethodIdent(), rootCauseInvocations, createTimerDataListOfRootCauses(rootCauseInvocations));

			// create new ProblemOccurrence
			ProblemOccurrence problem = new ProblemOccurrence(rootInvocationSequence, globalContext, problemContext.getCommonContext(), rootCause, causeStructure);
	...
	/**
	 * Converts all {@link #TimerData} of the method calls of the RootCause the a
	 * {@link #TimerDataProblemOccurence} and put them to a List. TODO: When we decide to aggregate
	 * these timerData without storing all single invocations we can remove this.
	 *
	 * @param rootCauseInvocations
	 *            Identified rootCauseInvocations
	 * @return Map of underlying methodsNames mapped to the {@link #timerData} of the rootCause.
	 */
	private List<TimerDataProblemOccurence> createTimerDataListOfRootCauses(AggregatedDiagnosisInvocationData rootCauseInvocations) {
		List<TimerDataProblemOccurence> timerDataPerMethod = new ArrayList<TimerDataProblemOccurence>();
		for (InvocationSequenceData invocation : rootCauseInvocations.getRawInvocationsSequenceElements()) {
			timerDataPerMethod.add(new TimerDataProblemOccurence(invocation));
		}
		return timerDataPerMethod;
	}
