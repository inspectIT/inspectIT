...
public class ProblemContextRule {
	...
	public CauseCluster action() {
		...
		if (causeInvocations.size() > 1) {
			...
			return getCauseCluster(causeInvocations);
		} else {
	 ...
	 *            List of invocations to get the parent of it if is the only one itself it has no
	 *            parent.
	 * @return Returns the CauseCluster.
	 */
	private CauseCluster getCauseCluster(List<InvocationSequenceData> causeInvocations) {
