...
public class RegistrationServiceTest extends AbstractTestNGLogSupport {
	...
	public void registerJmxSensorTypeIdent() throws RemoteException {
		...
		PlatformIdent platformIdent = new PlatformIdent();
		when(platformIdentDao.load(platformId)).thenReturn(platformIdent);
		when(jmxSensorTypeIdentDao.findByExample((JmxSensorTypeIdent) anyObject())).thenReturn(Collections.<JmxSensorTypeIdent> emptyList());
		Mockito.doAnswer(new Answer<Object>() {
			@Override
			public Object answer(InvocationOnMock invocation) throws Throwable {
	...
	public void registerJmxSensorDefinitionDataIdent() throws RemoteException {
		final long jmxSensorId = 50;
		long platformId = 1;
		JmxDefinitionDataIdent jmxDataIdent = new JmxDefinitionDataIdent();
		...
		long registeredId = registrationService.registerJmxSensorDefinitionDataIdent(platformId, jmxDataIdent);
		assertThat(registeredId, is(equalTo(jmxSensorId)));
		...
		assertThat(jmxSensorArgument.getValue(), is(equalTo(jmxDataIdent)));
	}
