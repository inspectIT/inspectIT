...
public class ProfileUpdateEvent extends ApplicationEvent {
	...
	public Collection<AbstractClassSensorAssignment<?>> getRemovedSensorAssignments() {
		if (isProfileDeactivated()) {
			...
			Collection<AbstractClassSensorAssignment<?>> resultsBefore = new ArrayList<>();
			resultsBefore.addAll(before.getMethodSensorAssignments());
			resultsBefore.addAll(before.getExceptionSensorAssignments());
			...
			Collection<AbstractClassSensorAssignment<?>> resultsAfter = new ArrayList<>();
			resultsAfter.addAll(after.getMethodSensorAssignments());
			resultsAfter.addAll(after.getExceptionSensorAssignments());

			return Utils.subtractSafe(resultsBefore, resultsAfter);
		}
	}
	...
	public Collection<AbstractClassSensorAssignment<?>> getAddedSensorAssignments() {
		if (isProfileDeactivated()) {
			...
			return Utils.subtractSafe(resultsAfter, resultsBefore);
		}
	}
