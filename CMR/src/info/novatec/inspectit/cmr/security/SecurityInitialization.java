...
public class SecurityInitialization {
	...
	public void start() {
		...
		if (userDao.loadAll().isEmpty()) {
			...
			User admin;
			try {
				admin = new User(Permutation.hashString(Permutation.hashString("admin")), "admin", adminRole.getId(), false);
			...
			User guest;
			try {
				guest = new User(Permutation.hashString(Permutation.hashString("guest")), "guest", guestRole.getId(), false);
			} catch (NoSuchAlgorithmException e) {
				log.info("NoSuchAlgorithException: Failed to create password hash. User not added to database!");
				return;
			}
