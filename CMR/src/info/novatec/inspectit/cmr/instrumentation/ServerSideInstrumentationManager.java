 ...
 */
@Component
public class ServerSideInstrumentationManager implements ApplicationListener<AgentDeletedEvent> {
	...
	public AgentConfiguration register(List<String> definedIPs, String agentName, String version) throws BusinessException {
		...
		if (configurationHolder.isInitialized() && Objects.equals(environment, cachedEnvironment)) {
			// TODO isInitialized enough
			AgentConfiguration agentConfiguration = configurationHolder.getAgentConfiguration();
			Map<Collection<String>, InstrumentationResult> initial = classCache.getInstrumentationService().getInstrumentationResultsWithHashes();
	...
	public InstrumentationResult analyzeAndInstrument(long platformIdent, String hash, Type sentType) throws BusinessException {
		...
		final ConfigurationHolder configurationHolder = agentCacheEntry.getConfigurationHolder();
		...
		return classCache.getInstrumentationService().instrument(classType, configurationHolder.getAgentConfiguration(), configurationHolder.getInstrumentationAppliers());
	}
		...
		executor.submit(new Runnable() {
			...
			public void run() {
				for (Entry<Long, long[]> entry : methodToSensorMap.entrySet()) {
					registrationService.refreshMethodIdentToSensorType(entry.getKey(), entry.getValue());
				}
			}
		});
	}
