...
public class DefaultDataDaoImpl implements DefaultDataDao {
	...
	public List<JmxSensorValueData> getJmxDataOverview(JmxSensorValueData template, Date fromDate, Date toDate) {
		...
		Predicate platformIdentPredicate = cb.equal(sqRoot.get("platformIdent"), cb.parameter(Long.class, "platformIdent"));
		Predicate sensorTypeIdentPredicate = cb.equal(sqRoot.get("sensorTypeIdent"), cb.parameter(Long.class, "sensorTypeIdent"));
		...
		if (template.getJmxSensorDefinitionDataIdentId() > 0) {
			predicate = cb.and(predicate, cb.equal(sqRoot.get("jmxSensorDefinitionDataIdentId"), cb.parameter(Long.class, "jmxSensorDefinitionId")));
		}
		...
		TypedQuery<JmxSensorValueData> q = entityManager.createQuery(c);
		q.setParameter("platformIdent", template.getPlatformIdent());
		q.setParameter("sensorTypeIdent", template.getSensorTypeIdent());
		
		if (template.getJmxSensorDefinitionDataIdentId() > 0) {
			q.setParameter("jmxSensorDefinitionId", template.getJmxSensorDefinitionDataIdentId());
		}
