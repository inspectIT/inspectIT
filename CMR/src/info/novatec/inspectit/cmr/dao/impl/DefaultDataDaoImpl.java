...
public class DefaultDataDaoImpl extends HibernateDaoSupport implements DefaultDataDao {
	...
	public List<JmxSensorValueData> getJmxDataOverview(JmxSensorValueData template, Date fromDate, Date toDate) {
		...
		StringBuilder hql = new StringBuilder(
				"FROM JmxSensorValueData data WHERE id IN (SELECT MAX(sub.id) FROM JmxSensorValueData sub WHERE sub.platformIdent = :platformId AND sub.sensorTypeIdent = :sensorTypeIdent");

		if (template.getJmxSensorDefinitionDataIdentId() > 0) {
			hql.append(" AND sub.jmxSensorDefinitionDataIdentId = :jmxSensorDefinitionId");
		}
		...
		query.setLong("sensorTypeIdent", template.getSensorTypeIdent());

		if (template.getJmxSensorDefinitionDataIdentId() > 0) {
			query.setLong("jmxSensorDefinitionId", template.getJmxSensorDefinitionDataIdentId());
		}
