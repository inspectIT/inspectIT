...
public class ClassInstrumenterPerfTest {
	...
	public void instrument() throws InterruptedException, IOException {
		...
		ClassInstrumenter classInstrumenter = new ClassInstrumenter(classWriter, Collections.<MethodInstrumentationConfig> emptyList()) {
			...
			MethodInstrumentationConfig shouldInstrument(String name, String desc) {
				...
				if (instrumented <= maxMethods) {
					...
					SensorInstrumentationPoint sensorInstrumentationPoint = new SensorInstrumentationPoint();
					sensorInstrumentationPoint.setEnhancedExceptionSensor(enhancedExceptionSensor);
					config.setSensorInstrumentationPoint(sensorInstrumentationPoint);
					return config;
				}
				return null;
			}
