...
public class KryoNetConnectionTest extends TestBase {
	...
	public static class Register extends KryoNetConnectionTest {
		...
		public void register() throws Exception {
			...
			AgentConfiguration receivedAgentConfiguration = connection.register(agentName, version);
	...
	public static class Unregister extends KryoNetConnectionTest {
		...
		public void unregister() throws Exception {
			...
			connection.unregister(agentName);

			verify(agentService, times(1)).unregister(Mockito.<List<String>> any(), eq(agentName));
			verifyNoMoreInteractions(agentService);
		}
		...
		public void timeout() throws Exception {
			when(client.isConnected()).thenReturn(true);
			doThrow(TimeoutException.class).when(agentService).unregister(Mockito.<List<String>> any(), anyString());
			String agentName = "agentName";
		...
		public void remoteException() throws Exception {
			when(client.isConnected()).thenReturn(true);
			doThrow(RuntimeException.class).when(agentService).unregister(Mockito.<List<String>> any(), anyString());
			String agentName = "agentName";
		...
		public void businessException() throws Exception {
			when(client.isConnected()).thenReturn(true);
			doThrow(BusinessException.class).when(agentService).unregister(Mockito.<List<String>> any(), anyString());
			String agentName = "agentName";
	...
	public static class AnalyzeAndInstrument extends KryoNetConnectionTest {
		...
		public void analyzeAndInstrument() throws Exception {
			...
			when(client.isConnected()).thenReturn(true);
			doReturn(instrumentationResult).when(agentService).analyzeAndInstrument(anyLong(), anyString(), Mockito.<Type> any());
			long id = 7;
			String hash = "hash";
			Type type = mock(Type.class);

			InstrumentationResult receivedResult = connection.analyzeAndInstrument(id, hash, type);
			assertThat(receivedResult, is(instrumentationResult));

			verify(agentService, times(1)).analyzeAndInstrument(id, hash, type);
			verifyNoMoreInteractions(agentService);
		}
		...
		public void timeout() throws Exception {
			when(client.isConnected()).thenReturn(true);
			doThrow(TimeoutException.class).when(agentService).analyzeAndInstrument(anyLong(), anyString(), Mockito.<Type> any());
			long id = 7;
			String hash = "hash";
			Type type = mock(Type.class);

			try {
				connection.analyzeAndInstrument(id, hash, type);
			} catch (ServerUnavailableException e) {
				assertThat(e.isServerTimeout(), is(true));
				throw e;
			} finally {
		...
		public void remoteException() throws Exception {
			when(client.isConnected()).thenReturn(true);
			doThrow(RuntimeException.class).when(agentService).analyzeAndInstrument(anyLong(), anyString(), Mockito.<Type> any());
			long id = 7;
			String hash = "hash";
			Type type = mock(Type.class);
		...
		public void businessException() throws Exception {
			when(client.isConnected()).thenReturn(true);
			doThrow(BusinessException.class).when(agentService).analyzeAndInstrument(anyLong(), anyString(), Mockito.<Type> any());
			long id = 7;
			String hash = "hash";
			Type type = mock(Type.class);
