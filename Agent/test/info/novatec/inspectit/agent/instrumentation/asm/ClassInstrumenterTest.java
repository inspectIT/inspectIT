...
public class ClassInstrumenterTest extends AbstractInstrumentationTest {
	...
	protected static final String TEST_CLASS_FQN = "info.novatec.inspectit.agent.instrumentation.asm.InstrumentationTestClass";

	protected static final String EXCEPTION_TEST_CLASS_FQN = "info.novatec.inspectit.agent.instrumentation.asm.InstrumentationExceptionTestClass";

	protected static final String TEST_CLASS_LOADER_FQN = "info.novatec.inspectit.agent.instrumentation.asm.MyTestClassLoader";
	...
	protected static final Answer<MethodVisitor> METHOD_INSTRUMENTER_ANSWER = new Answer<MethodVisitor>() {
		...
		public MethodVisitor answer(InvocationOnMock invocation) throws Throwable {
			...
			RegisteredSensorConfig rsc = (RegisteredSensorConfig) invocation.getMock();
			return getMethodInstrumenter((MethodVisitor) arguments[0], (Integer) arguments[1], (String) arguments[2], (String) arguments[3], rsc.getId(), rsc.isEnhancedExceptionSensor());
		}
	};
	...
	protected static final Answer<MethodVisitor> CONSTRUCTOR_INSTRUMENTER_ANSWER = new Answer<MethodVisitor>() {
		...
		public MethodVisitor answer(InvocationOnMock invocation) throws Throwable {
			Object[] arguments = invocation.getArguments();
			RegisteredSensorConfig rsc = (RegisteredSensorConfig) invocation.getMock();
			return getConstructorInstrumenter((MethodVisitor) arguments[0], (Integer) arguments[1], (String) arguments[2], (String) arguments[3], rsc.getId(), rsc.isEnhancedExceptionSensor());
		}
	};
	...
	@Mock
	protected IHookDispatcher hookDispatcher;

	@Mock
	protected IAgent agent;

	@Mock
	protected RegisteredSensorConfig rsc;

	@Mock
	protected RegisteredSensorConfig rsc2;

	@Mock
	protected MethodInstrumentationConfig config;

	@Mock
	protected MethodInstrumentationConfig config2;

	protected ClassInstrumenter classInstrumenter;

	protected LoaderAwareClassWriter classWriter;
	...
	public class ShouldInstrument extends ClassInstrumenterTest {

		@Test
		public void matchesAll() {
			when(config.getTargetMethodName()).thenReturn("methodName");
			when(config.getReturnType()).thenReturn("void");
			List<String> parameters = new ArrayList<String>();
			parameters.add("java.lang.Object");
			parameters.add("int");
			parameters.add("long[]");
			when(config.getParameterTypes()).thenReturn(parameters);

			when(config2.getTargetMethodName()).thenReturn("methodName2");
			when(config2.getReturnType()).thenReturn("double");
			when(config2.getParameterTypes()).thenReturn(Collections.<String> emptyList());

			List<MethodInstrumentationConfig> rscList = new ArrayList<MethodInstrumentationConfig>();
			rscList.add(config);
			rscList.add(config2);

			classInstrumenter = new ClassInstrumenter(null, rscList);

			assertThat(classInstrumenter.shouldInstrument("methodName", "(Ljava/lang/Object;I[J)V"), is(config));
			assertThat(classInstrumenter.shouldInstrument("methodName2", "()D"), is(config2));
		}
		...
		public void doesNotMatchName() {
			...
			classInstrumenter = new ClassInstrumenter(null, Collections.singletonList(config));
		...
		public void doesNotMatchReturnType() {
			when(config.getTargetMethodName()).thenReturn("methodName");
			when(config.getReturnType()).thenReturn("double");
		...
		public void doesNotMatchParameters() {
			...
			when(config.getParameterTypes()).thenReturn(Collections.<String> emptyList());
	...
	public class Instrument extends ClassInstrumenterTest {
		...
		public void methodHookNoStatic() throws Exception {
			...
			when(rsc.getId()).thenReturn(methodId);
			doAnswer(METHOD_INSTRUMENTER_ANSWER).when(rsc).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc));
			when(classInstrumenter.shouldInstrument(eq(methodName), Mockito.<String> any())).thenReturn(config);
		...
		public void staticConstructor() throws Exception {
			...
			when(rsc.getId()).thenReturn(methodId);
			doAnswer(CONSTRUCTOR_INSTRUMENTER_ANSWER).when(rsc).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc));
			when(classInstrumenter.shouldInstrument(eq("<clinit>"), Mockito.<String> any())).thenReturn(config);
		...
		public void constructorNullParameter() throws Exception {
			...
			doAnswer(CONSTRUCTOR_INSTRUMENTER_ANSWER).when(rsc).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc));
			when(classInstrumenter.shouldInstrument(eq("<init>"), Mockito.<String> any())).thenReturn(config);
		...
		public void nestedConstructorBooleanOneParameter() throws Exception {
			...
			doAnswer(CONSTRUCTOR_INSTRUMENTER_ANSWER).when(rsc).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc));
			when(classInstrumenter.shouldInstrument("<init>", "(Z)V")).thenReturn(config);

			when(rsc2.getId()).thenReturn(nestedMethodId);
			doAnswer(CONSTRUCTOR_INSTRUMENTER_ANSWER).when(rsc2).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config2.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc2));
			when(classInstrumenter.shouldInstrument("<init>", "(Ljava/lang/String;)V")).thenReturn(config2);
		...
		public void exceptionThrowerIsInstrumented() throws Exception {
			...
			when(rsc.getId()).thenReturn(methodId);
			doAnswer(METHOD_INSTRUMENTER_ANSWER).when(rsc).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(rsc.isEnhancedExceptionSensor()).thenReturn(true);
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc));
			when(classInstrumenter.shouldInstrument(eq(methodName), Mockito.<String> any())).thenReturn(config);
		...
		public void exceptionThrowerAndHandlerAreInstrumented() throws Exception {
			...
			doAnswer(METHOD_INSTRUMENTER_ANSWER).when(rsc2).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(rsc2.isEnhancedExceptionSensor()).thenReturn(true);
			when(config2.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(rsc2));
			when(classInstrumenter.shouldInstrument(eq(innerMethodName), Mockito.<String> any())).thenReturn(config2);
		...
		public void classLoadingDelegationActiveLoadClass() throws Exception {
			...
			FunctionalInstrumentationPoint functionalInstrumentationPoint = mock(FunctionalInstrumentationPoint.class);
	...
	protected void prepareWriter(ClassReader cr, ClassLoader classLoader) {
		...
		classInstrumenter.instrumentationConfigs = Collections.emptyList();
	}
