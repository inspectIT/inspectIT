...
public class ClassInstrumenterTest extends AbstractInstrumentationTest {
	...
	protected static final Answer<MethodVisitor> METHOD_INSTRUMENTER_ANSWER = new Answer<MethodVisitor>() {
		...
		public MethodVisitor answer(InvocationOnMock invocation) throws Throwable {
			...
			return getMethodInstrumenter((MethodVisitor) arguments[0], (Integer) arguments[1], (String) arguments[2], (String) arguments[3], sip.getId(), sip.isEnhancedExceptionSensor());
		}
	};
	...
	protected static final Answer<MethodVisitor> CONSTRUCTOR_INSTRUMENTER_ANSWER = new Answer<MethodVisitor>() {
		...
		public MethodVisitor answer(InvocationOnMock invocation) throws Throwable {
			Object[] arguments = invocation.getArguments();
			SensorInstrumentationPoint sip = (SensorInstrumentationPoint) invocation.getMock();
			return getConstructorInstrumenter((MethodVisitor) arguments[0], (Integer) arguments[1], (String) arguments[2], (String) arguments[3], sip.getId(), sip.isEnhancedExceptionSensor());
		}
	};
	...
	public class ShouldInstrument extends ClassInstrumenterTest {
		...
		public void matchesAll() {
			...
			List<MethodInstrumentationConfig> sipList = new ArrayList<MethodInstrumentationConfig>();
			sipList.add(config);
			sipList.add(config2);

			classInstrumenter = new ClassInstrumenter(null, sipList);
	...
	public class Instrument extends ClassInstrumenterTest {
		...
		public void methodHookNoStatic() throws Exception {
			...
			doAnswer(METHOD_INSTRUMENTER_ANSWER).when(sip).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
		...
		public void staticConstructor() throws Exception {
			...
			doAnswer(CONSTRUCTOR_INSTRUMENTER_ANSWER).when(sip).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(sip));
		...
		public void nestedConstructorBooleanOneParameter() throws Exception {
			...
			doAnswer(CONSTRUCTOR_INSTRUMENTER_ANSWER).when(sip2).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(config2.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(sip2));

			ClassReader cr = new ClassReader(TEST_CLASS_FQN);
			prepareWriter(cr, null, config, config2);
			cr.accept(classInstrumenter, ClassReader.SKIP_FRAMES | ClassReader.SKIP_DEBUG);
			assertThat(classInstrumenter.isByteCodeAdded(), is(true));
			byte b[] = classWriter.toByteArray();
		...
		public void exceptionThrowerIsInstrumented() throws Exception {
			...
			doAnswer(METHOD_INSTRUMENTER_ANSWER).when(sip).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(sip.isEnhancedExceptionSensor()).thenReturn(true);
			when(config.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(sip));
		...
		public void exceptionThrowerAndHandlerAreInstrumented() throws Exception {
			...
			doAnswer(METHOD_INSTRUMENTER_ANSWER).when(sip2).getMethodVisitor(Mockito.<MethodVisitor> any(), anyInt(), anyString(), anyString());
			when(sip2.isEnhancedExceptionSensor()).thenReturn(true);
			when(config2.getAllInstrumentationPoints()).thenReturn(Collections.<IMethodInstrumentationPoint> singleton(sip2));
	...
	protected void prepareWriter(ClassReader cr, ClassLoader classLoader, MethodInstrumentationConfig... configs) {
		classWriter = new LoaderAwareClassWriter(cr, ClassWriter.COMPUTE_FRAMES, classLoader);
		classInstrumenter = new ClassInstrumenter(classWriter, Arrays.asList(configs));
