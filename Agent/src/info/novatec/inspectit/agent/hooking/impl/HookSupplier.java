...
public class HookSupplier implements IHookSupplier {
	...
	/**
	 * The map for fast getting of the MethodSensorTypeConfigs.
	 */
	private final Map<Long, MethodSensorTypeConfig> sensorTypeConfigsMap = new HashMap<Long, MethodSensorTypeConfig>();

	/**
	 * The map for fast getting of the hooks.
	 */
	private final Map<Long, IHook> hookMap = new HashMap<Long, IHook>();
	...
	public void sensorInitialized(AbstractMethodSensor methodSensor) {
		...
		hookMap.put(sensorTypeConfig.getId(), methodSensor.getHook());
	}
	 ...
	 * {@inheritDoc}
	 */
	public IHook getHook(long id) {
		return hookMap.get(Long.valueOf(id));
	}
