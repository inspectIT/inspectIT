...
public class HookDispatcher implements IHookDispatcherMapper, IHookDispatcher {
	 ...
	 */
	private final NonBlockingHashMapLong<RegisteredSensorConfig> hooks = new NonBlockingHashMapLong<RegisteredSensorConfig>();
	...
	public void dispatchMethodBeforeBody(long id, Object object, Object[] parameters) {
		if (!executionMarker.isActive()) {
			try {
				...
				try {
					RegisteredSensorConfig rsc = hooks.get(Long.valueOf(id));
					...
					if (rsc.isStartsInvocation()) {
						...
						if (null == invocationSequenceHolder.get()) {
							...
							invocationSequenceHolder.set(hookSupplier.getMethodHook(invocationSensorTypeConfig.getId()));
						}
	...
	public void dispatchOnThrowInBody(long id, Object object, Object[] parameters, Object exceptionObject) {
		if (!executionMarker.isActive()) {
			try {
				...
				IExceptionSensorHook exceptionHook = (IExceptionSensorHook) hookSupplier.getMethodHook(exceptionSensorTypeConfig.getId());
