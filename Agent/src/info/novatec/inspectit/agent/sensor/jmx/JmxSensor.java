...
/**
 * This class provides information about the, in the config-file, specified MBeans of the
 * MBeanServer.
 * 
...
public class JmxSensor implements IJmxSensor, InitializingBean {
	 ...
	 */
	private Map<String, ObjectName> nameStringToObjectName = new HashMap<String, ObjectName>();
		...
		for (Iterator<UnregisteredJmxConfig> iterator = unregisteredJmxConfigs.iterator(); iterator.hasNext();) {
			...
			try {
				...
				for (ObjectName objectName : fittingMBeans) {
					String mBeanName = objectName + attributeNameExpression;
					if (!registeredJmxSensorConfigs.containsKey(mBeanName)) {
						...
						for (MBeanAttributeInfo mBeanAttributeInfo : attributes) {
							if (mBeanAttributeInfo.getName().equals(attributeNameExpression)) {
								...
								idManager.registerJmxDefinitionDataIdent(jsc);
								registeredJmxSensorConfigs.put(mBeanName, jsc);
								activeAttributes.put(mBeanName, jsc);
								nameStringToObjectName.put(objectName.toString(), objectName);
								break;
							}
						}
					} else if (!activeAttributes.containsKey(mBeanName)) {
						MBeanAttributeInfo[] attributes = mBeanServer.getMBeanInfo(objectName).getAttributes();
						for (MBeanAttributeInfo mBeanAttributeInfo : attributes) {
							if (mBeanAttributeInfo.getName().equals(attributeNameExpression)) {
								// reactivates an attribute to prevent re-occurring exceptions if a
								// mbean is removed during runtime.
								activeAttributes.put(mBeanName, registeredJmxSensorConfigs.get(mBeanName));
								break;
							}
						}
					}
