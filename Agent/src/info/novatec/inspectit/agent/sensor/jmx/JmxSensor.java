...
public class JmxSensor implements IJmxSensor, InitializingBean {
	...
	public void update(ICoreService coreService, long sensorTypeIdent) {
		// Check if the registerMBeans method should be invoked. Here, we check only for equals zero
		// because we resetting the counter if it is hitting the defined interval.
		if (updateCounter == 0) {
		...
		// increase the updateCounter and reset it if it's necessary
		updateCounter++;
		if (updateCounter >= REGISTER_BEAN_INTERVAL) {
			updateCounter = 0;
		}
	}
