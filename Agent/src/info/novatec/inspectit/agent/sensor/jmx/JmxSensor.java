...
public class JmxSensor implements IJmxSensor, InitializingBean {
	...
	/**
	 * Defines the interval after how many calls of the {@link #update(ICoreService, long)} method
	 * the {@link JmxSensor#collectData(ICoreService, long)} method should be invoked.
	 */
	private static final int DATA_COLLECT_INTERVAL = 5;

	/**
	 * Defines the interval after how many calls of the {@link #update(ICoreService, long)} method
	 * the {@link #registerMBeans()} method should be invoked.
	 */
	private static final int REGISTER_BEAN_INTERVAL = 60;
	...
	public void update(ICoreService coreService, long sensorTypeIdent) {
		...
		if (updateCounter % REGISTER_BEAN_INTERVAL == 0) {
		...
		if (updateCounter % DATA_COLLECT_INTERVAL == 0) {
			collectData(coreService, sensorTypeIdent);
		}

		// increase the updateCounter
		updateCounter++;
	}
					...
					if (!registeredJmxSensorConfigs.containsKey(mBeanAttributeKey)) {
						...
						for (MBeanAttributeInfo mBeanAttributeInfo : attributes) {
							if (mBeanAttributeInfo.getName().equals(attributeNameExpression)) {
								...
								String attributeType = mBeanAttributeInfo.getType();
								if (attributeType.startsWith("[L") && attributeType.endsWith(";")) {
									jsc.setmBeanAttributeType(attributeType.substring(2, attributeType.length() - 1) + "[]");
								} else {
									jsc.setmBeanAttributeType(attributeType);
								}
