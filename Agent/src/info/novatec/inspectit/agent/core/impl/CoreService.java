...
public class CoreService implements ICoreService, InitializingBean, DisposableBean {
	...
	public void addExceptionSensorData(long sensorTypeIdent, long throwableIdentityHashCode, ExceptionSensorData exceptionSensorData) {
		...
		// we always only save the first data object, because this object
		// contains the nested
		// objects to create the whole exception tree
		if (exceptionSensorData.getExceptionEvent().equals(ExceptionEvent.CREATED)) {
			// if a data object with the same hash code was already created,
			// then it has to be
			// removed, because it was created from a constructor delegation.
			// For us only the
			...
			while (!isInterrupted()) {
				...
				synchronized (this) {
					try {
						...
						break; // we were interrupted during waiting and close
								// ourself down.
					}
				}
				...
				if (!bufferStrategy.hasNext()) {
					synchronized (this) {
						try {
							...
							break; // we were interrupted during waiting and
									// close ourself down.
						}
					}
				}
	...
	private class ShutdownHookSender extends Thread {
		...
		public void run() {
			...
			// wait for the shutdown of the preparing thread and sending thread
			// to ensure thread
			// safety on the entities used for preparing and sending. If we get
			// interrupted while
			// waiting, then we stop the ShutdownHook completely. We'll wait
			// only 10 seconds as
			...
			// Try to send data for the last time. We do not set a timeout here,
			// the user can simply
