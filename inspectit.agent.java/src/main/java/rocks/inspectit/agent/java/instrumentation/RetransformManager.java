	...
	private void processInstrumentationDefinitions(Collection<InstrumentationDefinition> instrumentationDefinitions) {
		...
		for (Class<?> clazz : loadedClasses) {
			if (instrumentationDefinitionMap.containsKey(clazz.getCanonicalName())) {
				InstrumentationDefinition instrumentationDefinition = instrumentationDefinitionMap.get(clazz.getCanonicalName());
				...
				if (log.isDebugEnabled()) {
					log.debug("|-{} (is instrumented: {})", clazz.getCanonicalName(), !instrumentationDefinition.getMethodInstrumentationConfigs().isEmpty());
				}
			}
		}
