...
public class FutureCallbackProxy implements IProxySubject {
	 ...
	 */
	private static final CachedMethod<Void> M_COMPLETED = new CachedMethod<Void>("org.apache.http.concurrent.FutureCallback", "completed", Object.class);

	/**
	 * See {@link org.apache.http.concurrent.FutureCallback#failed(java.lang.Exception)}.
	 */
	private static final CachedMethod<Void> M_FAILED = new CachedMethod<Void>("org.apache.http.concurrent.FutureCallback", "failed", Exception.class);

	/**
	 * See {@link org.apache.http.concurrent.FutureCallback#cancelled()}.
	 */
	private static final CachedMethod<Void> M_CANCELLED = new CachedMethod<Void>("org.apache.http.concurrent.FutureCallback", "cancelled");
	...
	public void completed(Object response) {
		...
		if (originalCallback != null) {
			M_COMPLETED.callSafe(originalCallback, response);
		}
	}
	...
	public void failed(Object exception) {
		...
		if (originalCallback != null) {
			M_FAILED.callSafe(originalCallback, exception);
		}
	}
		...
		if (spanStore != null) {
			spanStore.finishSpan(new TagsProvidingAdapter() {
				...
				public Map<String, String> getTags() {
					return ImmutableMap.of("canceled", "true");
				}
			});
		}

		if (originalCallback != null) {
			M_CANCELLED.callSafe(originalCallback);
		}
	}
}
