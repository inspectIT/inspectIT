 ...
 * on binary data as long as the encoding is known. Simultaneously, this injector also accepts
 * already decoded Character data just as the {@link StreamedHTMLScriptInjector}.
 *
 * @author Jonas Kunz
 *
 */
public class DecodingHtmlScriptInjector extends StreamedHTMLScriptInjector {
	...
	private CharSequence decodeWithLeftOver(byte[] data, int offset, int length) {
		fetchCoders();
		if ((charEncoder == null) || (charDecoder == null)) {
