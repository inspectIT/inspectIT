...
public class HTMLTokenParser {
	...
	/**
	 * Specification of the results with which ther parser can terminate.
	 *
	 * @author Jonas Kunz
	 *
	 */
	public enum Result {
		/**
		 * The parser has ran into the end of the proided character buffer and could not complete
		 * reading the token.
		 */
		INCOMPLETE,

		/**
		 * The provided source html contained syntax errors, therefore no toke nwas parsed.
		 */
		FAILURE,
	...
	/**
	 * Internal status storing thew progress in parsing the token.
	 *
	 * @author Jonas Kunz
	 *
	 */
	...
	/**
	 * Tries to read a token based on the current position within the underyling html source. This
	 * results in {@link Result#SUCCESS} if the parsed succeeded in parsing a token, or
	 * {@link Result#FAILURE} if the html source has an unkown or invalid syntax. If the parser
	 * reached the end of the underyling html source without completing parsing a token,
	...
	/**
	 * Tries to scan for an opening brace after skipping all preceeding whitespace characters. A
	...
	private boolean scanTagType() {
		...
		if (caret.startsWithCheckCase("!--")) {
			...
			// we also allow a '?' or '!' at the beginnig because of !DOCTYPE and ?XML.
