	...
	public boolean isUsingRetransformation() {
		...
		try {
			...
			if (LOG.isWarnEnabled()) {
				LOG.warn("Class retransformation strategy could be set. Using the default retransformation stretegy: " + RetransformationStrategy.DISABLE_RETRANSFORMATION_ON_IBM_JVM);
			}
			retransformationStrategy = RetransformationStrategy.DISABLE_RETRANSFORMATION_ON_IBM_JVM;
		}

		return (retransformationStrategy == RetransformationStrategy.ALWAYS_USE_RETRANSFORMATION)
				|| ((retransformationStrategy == RetransformationStrategy.DISABLE_RETRANSFORMATION_ON_IBM_JVM) && (UnderlyingSystemInfo.JVM_PROVIDER != JvmProvider.IBM));
	}
