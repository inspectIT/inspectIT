...
public class ReflectionCache {
	...
	/**
	 * * Gets the given field on the given class. This method uses the cache to only lookup the
	 * field if the same field was not looked up before for the same class.
	 * <p>
	 * <b> Known limitation: The field name is used as key and not combined with the class, so you
	 * cannot cache fields with the same name on different classes. The reason is that we do not
	 * need this feature right now and it would complicate and slow down the cache. </b>
	 *
	 * @param clazz
