...
public final class RuntimeLinker implements IRuntimeLinker {
	...
	/**
	 * Prefix added to the proxies to prevent issues with classloader delegation. Not that this
	 * pattern is also present in the exclude-classes.xml.
	 */
	private static final String PROXY_CLASS_NAME_PREFIX = "proxied.";

	/**
	 * The suffix used for naming proxy classes. Not that this pattern is also present in the
	 * exclude-classes.xml.
	 */
			...
			synchronized (classLoaderMap) {
				...
				if (proxyClassInfo == null) {
					...
					try {
						...
						if (owner == null) {
							owner = ClassLoader.getSystemClassLoader();
						}
