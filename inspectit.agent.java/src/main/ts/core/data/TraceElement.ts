...
abstract class TraceElement extends BeaconElement {
    ...
    /**
     * Cosntant representing that the executionOrderIndex is not known.
     * This is for example the case for resource laod requests.
     */
    public static readonly UNKNOWN_EXECUTION_ORDER_INDEX = -1;

    /**
     * Holds the threshold when elements become relevant due to their execution duration.
     */
    private static readonly MINIMUM_DURATION_FOR_RELEVANCY = SETTINGS.relevancyThreshold;

    /**
     * Reference Pool to allow Elemetns to reference hteir parent for the propagation of markRelevant().
     * These references must be soft to prevent memory leaks in case of traces of infinite depth,
     * For example like recursive setTimeout calls.
     */
    private static readonly REF_POOL = new SoftReferencesPool<TraceElement>(1000);

    /**
     * Counter used for assigning execution order indices.
     */
     ...
     */
    private parentRef: SoftReference<TraceElement> = new SoftReference(TraceElement.REF_POOL);
    ...
    public buildTrace<T>(captureTimings: boolean, functionToTrace?: () => T) {
        ...
        TraceBuilder.enterElement(this);
        if (captureTimings) {
