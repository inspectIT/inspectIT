...
import rocks.inspectit.agent.java.connection.impl.AgentMessageFetcher;
...
public class AgentMessageFetcherTest extends TestBase {
	...
	public static class PostConstruct extends AgentMessageFetcherTest {
		...
		public void successful() {
			...
			verify(executorService).scheduleAtFixedRate(messageFetcher, 5L, 5L, TimeUnit.SECONDS);
	...
	public static class Run extends AgentMessageFetcherTest {
		...
		public void successful() throws IdNotAvailableException, ServerUnavailableException {
			...
			List<AbstractAgentMessage> messages = Arrays.asList(mock(AbstractAgentMessage.class));
			...
			assertThat(messages, is(equalTo(eventCaptor.getValue().getAgentMessages())));
		}
