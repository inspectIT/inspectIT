...
public class HttpClientBuilderHookTest extends TestBase {
	...
	public static class BeforeBody extends HttpClientBuilderHookTest {
		...
		public void mustCallToInvokeMethodOfCacheWithTheProperParameters() throws Exception {
			...
			// spy hook
			HttpClientBuilderHook spyHook = spy(hook);
			doReturn(HttpRequestInterceptor.class).when(spyHook).getRequestInterceptorClass(object.getClass().getClassLoader());

			spyHook.beforeBody(METHOD_ID, object, parameters, ssc);
