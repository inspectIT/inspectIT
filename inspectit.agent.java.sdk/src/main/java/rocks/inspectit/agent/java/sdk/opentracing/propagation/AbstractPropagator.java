...
public abstract class AbstractPropagator<C> implements Propagator<C> {
	...
	public void inject(SpanContextImpl spanContext, C carrier) {
		...
		injectBaggage(carrier, PropagationConstants.SPAN_ID, ConversionUtil.toHexString(spanContext.getId()));
		injectBaggage(carrier, PropagationConstants.TRACE_ID, ConversionUtil.toHexString(spanContext.getTraceId()));
	...
	public SpanContextImpl extract(C carrier) {
		...
		if (notEmpty(idFromBaggage) && notEmpty(traceIdFromBaggage)) {
			try {
				long id = ConversionUtil.parseHexStringSafe(idFromBaggage);
				long traceId = ConversionUtil.parseHexStringSafe(traceIdFromBaggage);
