...
public class TracerImpl implements ExtendedTracer {
	 ...
	 * Logger for logging.
	 */
	private final TracerLogger logger;
	 ...
	 */
	public TracerImpl() {
		this(new SystemTimer(), new NoopReporter(), new NoopLogger(), false);
	}
	 ...
	 */
	public TracerImpl(Timer timer, Reporter reporter, TracerLogger logger, boolean setToTracerProvider) {
		if (null == timer) {
	...
	void spanEnded(SpanImpl span) {
		...
		if (stack.contains(span)) {
			...
			if (wrongEndOrder) {
				logger.warn("Ending of spans is not done in creation order. Thread context state can be affected.");
			}
		}
