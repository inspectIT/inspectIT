...
public class BusinessContextRecognitionProcessor extends AbstractCmrDataProcessor implements ApplicationListener<BusinessContextDefinitionUpdateEvent> {
	 ...
	 */
	@Autowired
	private IBusinessContextRegistryService businessContextMngmtService;
	...
	public void assignBusinessContext(InvocationSequenceData invocSequence) {
		...
		ApplicationData application = businessContextMngmtService.registerApplication(appDefinition);
		...
		BusinessTransactionData businessTransaction = businessContextMngmtService.registerBusinessTransaction(application, businessTxDefinition, businessTxName);
