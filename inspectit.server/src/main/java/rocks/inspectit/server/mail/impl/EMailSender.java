...
public class EMailSender implements IEMailSender {
		...
		if (null != smtpPropertiesString) {
			...
			for (String property : strArray) {
				...
				if ((equalsIndex > 0) && (equalsIndex < (property.length() - 1))) {
					getAdditionalProperties().put(property.substring(0, equalsIndex), property.substring(equalsIndex + 1));
				}
			}
		}
	}
	...
	protected void checkConnection() {
		try {
			Session session = Session.getInstance(getAdditionalProperties(), new DefaultAuthenticator(smptUser, smptPassword));
