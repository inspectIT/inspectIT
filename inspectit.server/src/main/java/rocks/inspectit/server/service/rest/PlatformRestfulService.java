...
public class PlatformRestfulService {
	...
	@ResponseBody
	public String getStatus(@RequestParam(value = "agentId", required = true) long agentId) throws BusinessException {
		PlatformIdent paltformIdent = globalDataAccessService.getCompleteAgent(agentId);
		Map<PlatformIdent, AgentStatusData> agents = globalDataAccessService.getAgentsOverview();
		if (!agents.containsKey(paltformIdent)) {
			throw new BusinessException("Agent with ID " + agentId + " does not exist!", AgentManagementErrorCodeEnum.AGENT_DOES_NOT_EXIST);
		}
		return globalDataAccessService.getAgentsOverview().get(paltformIdent).getAgentConnection().toString();
	}
	...
	@ResponseBody
	public Set<MethodIdent> getMethods(@RequestParam(value = "agentId", required = true) long agentId) throws BusinessException {
	...
	@ResponseBody
	public Set<SensorTypeIdent> getSensors(@RequestParam(value = "agentId", required = true) long agentId) throws BusinessException {
