...
public class ThresholdChecker {
	...
	public void checkThreshold(AlertingState alertingState) {
		if (log.isDebugEnabled()) {
			log.debug("||-Check threshold defined by alerting definition '{}'.", alertingState.getAlertingDefinition().getDescriptiveName());
		}
		...
		if (lastCheckTime < 0) {
			lastCheckTime = currentTime - TimeUnit.MINUTES.toMillis(alertingState.getAlertingDefinition().getTimerange());
