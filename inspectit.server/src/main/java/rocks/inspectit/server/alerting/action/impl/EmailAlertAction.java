...
public class EmailAlertAction implements IAlertAction {
	 ...
	 */
	@Override
	public void onStarting(AlertingState alertingState, double violationValue) {
		if (log.isDebugEnabled()) {
			log.debug("||-Sending e-mail because of starting an alert specified by: {}", alertingState.getAlertingDefinition().getDescriptiveName());
		}
		...
		try {
			...
			if (AlertingUtils.isBusinessTransactionAlert(alertingState.getAlertingDefinition())) {
				htmlBody = getEmailBodyForAlert(AlertEMailTemplateType.HTML_BUSINESS_TX_ALERT_OPEN, alertingState, violationValue);
				textBody = getEmailBodyForAlert(AlertEMailTemplateType.TXT_BUSINESS_TX_ALERT_OPEN, alertingState, violationValue);
			} else {
				htmlBody = getEmailBodyForAlert(AlertEMailTemplateType.HTML_ALERT_OPEN, alertingState, violationValue);
				textBody = getEmailBodyForAlert(AlertEMailTemplateType.TXT_ALERT_OPEN, alertingState, violationValue);
			}
	...
	public void onEnding(AlertingState alertingState) {
		...
		try {
			String subject = "Alert - Threshold '" + alertingState.getAlertingDefinition().getName() + "' has been closed";
			String htmlBody;
			String textBody;
	...
	private String getEmailBodyForAlert(AlertEMailTemplateType templateType, AlertingState alertingState, double violationValue) throws IOException {
		...
		properties.put(AlertEMailTemplateType.Placeholders.THRESHOLD, String.valueOf(alertingState.getAlertingDefinition().getThreshold()));
		...
		properties.put(AlertEMailTemplateType.Placeholders.VIOLATION_VALUE, String.valueOf(violationValue));
		...
		properties.put(AlertEMailTemplateType.Placeholders.EXTREME_VALUE, String.valueOf(alertingState.getTotalExtremeValue()));
