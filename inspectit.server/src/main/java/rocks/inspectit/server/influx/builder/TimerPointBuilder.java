...
public class TimerPointBuilder extends DefaultDataPointBuilder<TimerData> {
	...
	protected void addTags(TimerData data, Builder builder) {
		...
		if (null != methodIdent) {
			builder.tag(Tags.METHOD_NAME, methodIdent.getMethodName());
			builder.tag(Tags.CLASS_FQN, methodIdent.getFQN());
			builder.tag(Tags.METHOD_SIGNATURE, methodIdent.getFullyQualifiedMethodSignature());
		}
	}
