...
public class EMailSenderTest extends TestBase {
	...
	public static class CheckConnection extends EMailSenderTest {
		...
		@BeforeMethod
		public void beforeMethod() throws NoSuchProviderException {
			sender.smptHost = "host";
			sender.smptPort = 123;
			sender.smptUser = "user";
			sender.smptPassword = "passwd";

			mockTransport = mock(Transport.class);
			senderSpy = spy(sender);

			doReturn(mockTransport).when(senderSpy).getTransport();
		}

		@Test
		public void successful() throws MessagingException {
			senderSpy.checkConnection();

			verify(mockTransport).connect(sender.smptHost, sender.smptPort, sender.smptUser, sender.smptPassword);

			assertThat(senderSpy.isConnected(), is(true));
		}

		@Test
		public void connectionFailed() throws MessagingException {
			doThrow(MessagingException.class).when(mockTransport).connect(any(String.class), any(Integer.class), any(String.class), any(String.class));

			senderSpy.checkConnection();

			assertThat(senderSpy.isConnected(), is(false));
		}

		@Test
		public void authenticationFailed() throws MessagingException {
			doThrow(AuthenticationFailedException.class).when(mockTransport).connect(any(String.class), any(Integer.class), any(String.class), any(String.class));
	...
	public static class PrepareHtmlEmail extends EMailSenderTest {
		...
		public void beforeMethod() {
			...
			doNothing().when(senderSpy).checkConnection();
		}
