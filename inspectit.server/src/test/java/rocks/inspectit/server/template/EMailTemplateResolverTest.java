...
public class EMailTemplateResolverTest extends TestBase {
	...
	public static class ResolveTemplate extends EMailTemplateResolverTest {
		...
		public void isBusinessTransaction() throws IOException, ParseException {
			...
			Mockito.when(alertingState.getExtremeValue()).thenReturn(100D);
			Mockito.when(alertingDefinition.getName()).thenReturn("name");
			...
			Mockito.when(alertingDefinition.getThreshold()).thenReturn(200D);
			Mockito.when(alertingDefinition.getTags()).thenReturn(tags);
			Mockito.when(alertingDefinition.getNotificationEmailAddresses()).thenReturn(Arrays.asList("test@example.com"));
			Mockito.when(alert.getId()).thenReturn("id");
			Mockito.when(alert.getStartTimestamp()).thenReturn(10L);
			Mockito.when(templateManager.resolveTemplate(eq(AlertEMailTemplateType.HTML_ALERT_OPEN), any(Map.class))).thenReturn("tplt");
			...
			Mockito.verify(templateManager).resolveTemplate(eq(AlertEMailTemplateType.HTML_ALERT_OPEN), propertiesCaptor.capture());
			Mockito.verifyNoMoreInteractions(templateManager);
			Mockito.verify(alertingState).getAlertingDefinition();
			Mockito.verify(alertingState).getAlert();
			Mockito.verify(alertingState, times(2)).getExtremeValue();
			Mockito.verifyNoMoreInteractions(alertingState);
			Mockito.verify(alertingDefinition).getName();
			Mockito.verify(alertingDefinition, times(4)).getMeasurement();
			Mockito.verify(alertingDefinition, times(4)).getField();
			Mockito.verify(alertingDefinition).getThreshold();
			Mockito.verify(alertingDefinition, times(3)).getTags();
			Mockito.verifyNoMoreInteractions(alertingDefinition);
			Mockito.verify(alert).getStartTimestamp();
			Mockito.verify(alert).getStopTimestamp();
			Mockito.verify(alert).getId();
		...
		public void noBusinessTransaction() throws IOException, ParseException {
			...
			Mockito.when(alertingState.getExtremeValue()).thenReturn(100D);
			Mockito.when(alertingDefinition.getName()).thenReturn("name");
			Mockito.when(alertingDefinition.getMeasurement()).thenReturn("measurement");
			Mockito.when(alertingDefinition.getField()).thenReturn("field");
			Mockito.when(alertingDefinition.getThreshold()).thenReturn(200D);
			Mockito.when(alertingDefinition.getTags()).thenReturn(tags);
			Mockito.when(alertingDefinition.getNotificationEmailAddresses()).thenReturn(Arrays.asList("test@example.com"));
			Mockito.when(alert.getId()).thenReturn("id");
			...
			Mockito.verifyNoMoreInteractions(alertingState);
			Mockito.verify(alertingDefinition).getName();
			Mockito.verify(alertingDefinition, times(2)).getMeasurement();
		...
		public void noBusinessTransactionIsText() throws IOException, ParseException {
			...
			Mockito.when(alert.getId()).thenReturn("id");
			Mockito.when(alert.getStartTimestamp()).thenReturn(10L);
			Mockito.when(templateManager.resolveTemplate(eq(AlertEMailTemplateType.TXT_ALERT_OPEN), any(Map.class))).thenReturn("tplt");

			String result = resolver.resolveTemplate(AlertEMailTemplateType.TXT_ALERT_OPEN, alertingState);

			assertThat(result, is("tplt"));
			ArgumentCaptor<Map> propertiesCaptor = ArgumentCaptor.forClass(Map.class);
			Mockito.verify(templateManager).resolveTemplate(eq(AlertEMailTemplateType.TXT_ALERT_OPEN), propertiesCaptor.capture());
			Mockito.verifyNoMoreInteractions(templateManager);
			Mockito.verify(alertingState).getAlertingDefinition();
			Mockito.verify(alertingState).getAlert();
			Mockito.verify(alertingState, times(2)).getExtremeValue();
		...
		public void noBusinessTransactionIsTextHasStopped() throws IOException, ParseException {
			...
			Mockito.when(alert.getId()).thenReturn("id");
			Mockito.when(alert.getStartTimestamp()).thenReturn(10L);
			Mockito.when(alert.getStopTimestamp()).thenReturn(20L);
			Mockito.when(alert.getClosingReason()).thenReturn(AlertClosingReason.ALERT_RESOLVED);
			Mockito.when(templateManager.resolveTemplate(eq(AlertEMailTemplateType.TXT_ALERT_CLOSED), any(Map.class))).thenReturn("tplt");

			String result = resolver.resolveTemplate(AlertEMailTemplateType.TXT_ALERT_CLOSED, alertingState);

			assertThat(result, is("tplt"));
			ArgumentCaptor<Map> propertiesCaptor = ArgumentCaptor.forClass(Map.class);
			Mockito.verify(templateManager).resolveTemplate(eq(AlertEMailTemplateType.TXT_ALERT_CLOSED), propertiesCaptor.capture());
			Mockito.verifyNoMoreInteractions(templateManager);
			Mockito.verify(alertingState).getAlertingDefinition();
			Mockito.verify(alertingState).getAlert();
			Mockito.verify(alertingState, times(2)).getExtremeValue();
			...
			Mockito.verifyNoMoreInteractions(alertingDefinition);
			Mockito.verify(alert).getStartTimestamp();
			Mockito.verify(alert, times(2)).getStopTimestamp();
			Mockito.verify(alert).getId();
			Mockito.verify(alert).getClosingReason();
			Mockito.verifyNoMoreInteractions(alert);
			//
			Map<String, String> map = propertiesCaptor.getValue();
			assertThat(map.get(Placeholders.ALERT_DEFINITION_NAME), is("name"));
