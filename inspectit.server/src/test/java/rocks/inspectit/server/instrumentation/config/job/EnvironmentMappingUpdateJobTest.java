...
public class EnvironmentMappingUpdateJobTest {
	...
	public void yesEnvironment() {
		Type[] typeArray = { mock(Type.class) };
		...
		doReturn(Arrays.asList(typeArray)).when(instrumentationService).addInstrumentationPoints(any(AgentConfig.class), any(Collection.class));
		...
		assertThat((Collection<Type>) typeCaptor.getValue(), hasItems(typeArray));
