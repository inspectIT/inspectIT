...
public class TimerMethodSensorInstrumentationApplier extends MethodSensorInstrumentationApplier {
	 ...
	 */
	private TimerMethodSensorAssignment timerAssignment;
	...
	protected void applyAssignment(AgentConfiguration agentConfiguration, RegisteredSensorConfig registeredSensorConfig) {
		...
		if (timerAssignment.isStartsInvocation()) {
			...
			if (null != invocationSensorConfig) {
				...
				if (registeredSensorConfig.addSensorId(invocationSensorTypeConfig.getId(), invocationSensorTypeConfig.getPriority())) {
					// if this is new id for the invocation sensor config then register mapping
					registrationService.addSensorTypeToMethod(invocationSensorTypeConfig.getId(), registeredSensorConfig.getId());
