...
public class BusinessTransactionDefinition implements IMatchingRuleProvider {
	...
	public int createBusinessTransactionId(String businessTransactionName) {
		if (getId() == 0) {
			...
			final int prime = 31;
			int result = 1;
			result = prime * result + businessTransactionName.hashCode();
	...
	private String extractNameDynamically(InvocationSequenceData invocSequence, ICachedDataService cachedDataService, int depth) {
		...
		if (null == name && null != invocSequence.getNestedSequences() && (getNameExtractionExpression().getMaxSearchDepth() < 0 || depth < getNameExtractionExpression().getMaxSearchDepth())) {
