			...
			public void run() {
				...
				if (forceDialog || (saveBehavior != OnSaveBehavior.DO_NOTHING)) {
					...
					if (CollectionUtils.isNotEmpty(updateAgents)) {
						...
						ProgressDialog<Void> dialog = new ProgressDialog<Void>("Updating instrumentation configurations..", IProgressMonitor.UNKNOWN) {
							@Override
						...
						try {
							...
							if (!dialog.wasSuccessful()) {
								InspectIT.getDefault().createErrorDialog("Unexpected exception occurred during update of the instrumentation configurations.", dialog.getThrownException(), -1);
							}
						} catch (Exception e) {
							InspectIT.getDefault().createErrorDialog("Unexpected exception occurred during update of the instrumentation configurations.", e, -1);
						}
					}
				}
			}
