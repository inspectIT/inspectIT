...
public class SelectiveRulesPart implements IMatchingRulesPart {
	 ...
	 *            changes.
	 */
	public SelectiveRulesPart(String title, Composite parent, IManagedForm managedForm, AbstractValidationManager<AbstractExpression> validationManager) {
		this.title = title;
		this.parent = parent;
	...
	private void createPart(boolean advanced) {
		if (advanced) {
			rulesView = new AdvancedMatchingRulesPart(title, validationManager);
		} else {
			rulesView = new SimpleMatchingRulesPart(title, parent, managedForm, validationManager);
		}
