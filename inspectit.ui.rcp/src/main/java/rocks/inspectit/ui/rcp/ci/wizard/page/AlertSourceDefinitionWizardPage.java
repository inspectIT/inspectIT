...
public class AlertSourceDefinitionWizardPage extends WizardPage {
	 ...
	 */
	private <E> E executeInBackground(final Callable<E> task) {
		BlockingJob<E> job = new BlockingJob<E>("Waiting for repository response..", task);
		return job.scheduleAndJoin();
	}
