...
public class EUMSettingsPart extends SectionPart implements IPropertyListener {
	...
	private void createPart(Section section, FormToolkit toolkit) {
		...
		createInfoLabel(mainComposite, toolkit, "Expert Option: If deactivated, the JS agent will not instrument any JS event listeners to prevent performance issues.");
		...
		createInfoLabel(mainComposite, toolkit,
				"Expert Option: If enabled, the clients will receive a minified version of the JS Agent which reduces the size and improves the performance. Should only be disabled for debugging.");
