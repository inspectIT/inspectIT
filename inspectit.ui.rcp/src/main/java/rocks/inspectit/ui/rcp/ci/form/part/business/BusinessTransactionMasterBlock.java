...
public class BusinessTransactionMasterBlock extends MasterDetailsBlock implements IFormPart, IPropertyListener {
		...
		tableViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				if (!event.getSelection().isEmpty() && event.getSource() == tableViewer) {
					...
					managedForm.fireSelectionChanged(BusinessTransactionMasterBlock.this, structuredSelection.size() == 1 ? event.getSelection() : StructuredSelection.EMPTY);
	...
	// */
	// @Override
	// public void validationStateChanged(boolean valid, ValidationControlDecoration<?>
	...
	// for (Entry<ValidatorKey, ValidationControlDecoration<?>> entry :
	// getValidationControlDecorators().entrySet()) {
	// if (null != entry.getKey().getBusinessTransactionDefinition() &&
	// entry.getKey().getBusinessTransactionDefinition().equals(key.getBusinessTransactionDefinition()))
	// {
		...
		if (null != tableViewer) {
			Display.getDefault().asyncExec(new Runnable() {
				...
				public void run() {
					...
					validationManager.updateValidationMessagesForBusinessTransactions(applicationDefinition.getBusinessTransactionDefinitions());
				}
			});
		}
	}
	...
	private class BusinessTransactionValidationManager extends AbstractValidationManager<BusinessTransactionDefinition> {
		...
		protected void showMessage(BusinessTransactionDefinition businessTxDefinition, Set<ValidationState> states) {
			...
			if (errorCount <= 0) {
				...
				errorsText = "One part contains a validation error";
			} else if (errorCount > 1) {
				errorsText = errorCount + " parts contain validation errors";
			}
