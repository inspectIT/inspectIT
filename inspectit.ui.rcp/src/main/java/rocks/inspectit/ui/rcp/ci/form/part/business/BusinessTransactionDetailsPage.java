...
public class BusinessTransactionDetailsPage implements IDetailsPage {
	...
	public void createContents(Composite parent) {
		...
		rulesPart = new SelectiveRulesPart(BUSINESS_TRANSACTION_MAPPING_PART_NAME, parent, formPage.getManagedForm(),
				new BusinessTransactionPartsValidationManager<AbstractExpression>(upstreamValidationManager));
		nameExtractionPart = new DynamicNameExtractionPart(parent, formPage.getManagedForm(), new BusinessTransactionPartsValidationManager<String>(upstreamValidationManager));
	}
	...
	private class BusinessTransactionPartsValidationManager<K> extends AbstractValidationManager<K> {
		...
		protected void notifyUpstream(K key, Set<ValidationState> states) {
			...
			String message = part + " (" + errorsText + ")";
			upstreamValidationManager.validationStateChanged(selectedBusinessTransaction, new ValidationState(key, errorCount <= 0, message));
		}
