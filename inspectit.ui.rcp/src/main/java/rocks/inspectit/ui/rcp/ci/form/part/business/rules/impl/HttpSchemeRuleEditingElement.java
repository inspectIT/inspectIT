...
/**
 * Editing element for a HTTP scheme matching expression.
 * 
 * @author Marius Oehler
 *
 */
public class HttpSchemeRuleEditingElement extends AbstractStringMatchingRuleEditingElement<HttpSchemeValueSource> {
	/**
	 * The name of the string source.
	 */
	private static final String SOURCE_NAME = "Scheme";

	/**
	 * Description text for the HTTP scheme matching rule.
	 */
	private static final String DESCRIPTION = "This rule applies if the scheme of the corresponding request\n" + "matches (equals, starts with, etc.) the specified String value.";
	 ...
	 *            changes.
	 */
	public HttpSchemeRuleEditingElement(StringMatchingExpression expression, boolean editable, AbstractValidationManager<AbstractExpression> upstreamValidationManager) {
		super(expression, MatchingRuleType.HTTP_SCHEME, DESCRIPTION, SOURCE_NAME, false, editable, upstreamValidationManager);
	}
	...
	protected boolean isValidExpression(StringMatchingExpression expression) {
		return expression.getStringValueSource() instanceof HttpSchemeValueSource;
	}
}
