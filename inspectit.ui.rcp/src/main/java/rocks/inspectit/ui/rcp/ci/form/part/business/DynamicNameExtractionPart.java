...
import rocks.inspectit.ui.rcp.validation.ControlValidationManager;
...
public class DynamicNameExtractionPart extends SectionPart {
	 ...
	 */
	private final ControlValidationManager<String> validationManager;

	/**
	 * Current selection of the string source.
	 */
	private MatchingRuleType stringSourceSelection = DEFAULT_MATCHING_RULE_TYPE;

	/**
	 * Indicates whether the default pattern has been selected (true), or a custom pattern is
	 * specified (false).
	 */
	...
	public DynamicNameExtractionPart(Composite parent, IManagedForm managedForm, AbstractValidationManager<String> validationManager) {
		...
		this.validationManager = new ControlValidationManager<String>(validationManager);
	 ...
	 *            the heading text
	 */
	private void createSectionHeading(String heading) {
		FormText headingText = toolkit.createFormText(mainComposite, false);
		GridData gridData = new GridData(SWT.FILL, SWT.CENTER, true, false, NUM_COLUMNS, 1);
		gridData.verticalIndent = 20;
		headingText.setLayoutData(gridData);
		...
		headingText.setText("<form><p><span color=\"header\" font=\"header\">" + heading + "</span></p></form>", true, false);
	}
	...
	private void stringSourceSelectionChanged() {
		...
		resetStringValueSection();

		// set new selection and create controls for the new selection
		searchDepthSpinner.setEnabled(true);
		searchInTraceCheckBox.setEnabled(true);
	...
	private void reset() {
		resetNamePatternSection();
		resetStringValueSection();
		resetSearchInTraceSection();

		optionalControls.clear();
		extractNameCheckbox.setSelection(false);
		stringSourceSelectionCombo.select(0);
		stringSourceSelection = DEFAULT_MATCHING_RULE_TYPE;
		setMainPartEnabled(false);
	}
	 ...
	 */
	private void resetNamePatternSection() {
	 ...
	 */
	private void resetStringValueSection() {
