...
public class SensorAssignmentMasterBlock extends MasterDetailsBlock implements IFormPart, IPartSelectionListener, ISelectionChangedListener, IPropertyListener {
	 ...
	 */
	private SensorAssigmentProfileData profileData;
	...
	public SensorAssignmentMasterBlock(FormPage formPage) {
		...
		Assert.isLegal(input.getProfile().getProfileData().isOfType(SensorAssigmentProfileData.class), "Given profile can not be opened with the exclude rules part.");

		this.profile = input.getProfile();
		this.profileData = profile.getProfileData().getIfInstance(SensorAssigmentProfileData.class);
	...
	public void propertyChanged(Object source, int propId) {
		if (propId == IEditorPart.PROP_INPUT) {
			...
			profileData = profile.getProfileData().getIfInstance(SensorAssigmentProfileData.class);
		}
	}
