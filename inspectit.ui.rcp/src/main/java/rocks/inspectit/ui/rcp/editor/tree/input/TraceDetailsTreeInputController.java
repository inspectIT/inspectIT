...
public class TraceDetailsTreeInputController extends AbstractTreeInputController {
	 ...
	 */
	private InvocationTreeContentProvider contentProvider;
	...
	public void setInputDefinition(InputDefinition inputDefinition) {
		...
		contentProvider = new InvocationTreeContentProvider(spanService, invocationDataAccessService);
		contentProvider.setBuildMode(Mode.ONLY_SPANS_WITH_SDK);
	...
	public void doRefresh(IProgressMonitor monitor, IRootEditor rootEditor) {
		...
		if (traceId != 0) {
			InvocationTreeBuilder builder = new InvocationTreeBuilder();
			final InvocationTreeElement tree = builder.setSpanService(spanService).setInvocationService(invocationDataAccessService).setTraceId(traceId).setMode(Mode.ONLY_SPANS_WITH_SDK).build();
			...
			Display.getDefault().asyncExec(new Runnable() {
				...
				public void run() {
					...
					rootEditor.setDataInput(Collections.singletonList(tree.getDataElement()));
				}
			});
		}
	...
	public Object[] getObjectsToSearch(Object treeInput) {
		return InvocationTreeUtil.getDataElements(contentProvider.getTree()).toArray();
	}
	...
	private StyledString getStyledTextForColumn(Span span, Column column) {
		InvocationTreeElement ite = contentProvider.getTree().lookup(span);
		...
		case DURATION:
			...
			if (ite.isConsideredAsync()) {
				durationString.append(TextFormatter.getWarningSign());
			}
			return durationString;
		case EXCLUSIVE:
			StyledString exclusive = new StyledString();
			exclusive.append(NumberFormatter.formatDouble(ite.getExclusiveDuration(), timeDecimalPlaces));
			exclusive.append(" (", StyledString.COUNTER_STYLER);
			int percentage = Math.round((float) ite.getExclusivePercentage() * 100);
	...
	private final class TraceDetailLabelProvider extends StyledCellIndexLabelProvider {
		...
		protected Image getColumnImage(Object element, int index) {
			...
			if (element instanceof Span) {
				Span span = (Span) element;
				InvocationTreeElement treeElement = contentProvider.getTree().lookup(span);
		...
		public String getToolTipText(Object element, int index) {
			...
			case DURATION:
				if (treeElement.isConsideredAsync()) {
