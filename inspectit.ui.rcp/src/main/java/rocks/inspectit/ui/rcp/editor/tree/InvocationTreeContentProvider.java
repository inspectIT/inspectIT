...
public class InvocationTreeContentProvider implements ITreeContentProvider {
	...
	public Object[] getElements(Object inputElement) {
		...
		// we return the parent data element if it is a calling span (=client span)
		if (InvocationTreeUtil.isChildOfSpan(element)) {
			Span parentSpan = (Span) element.getParent().getDataElement();
			if (parentSpan.isCaller()) {
				return new Object[] { parentSpan };
			}
		}
