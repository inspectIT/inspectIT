...
public class AlertSelectionDialog extends TitleAreaDialog {
	...
	private void validateInput() {
		...
		alert = StringUtils.isEmpty(alertId) ? null : cmrRepositoryDefinition.getAlertAccessService().getAlert(alertId); // NOPMD
		if (null == alert) {
			okButton.setEnabled(false);
			if (StringUtils.isEmpty(alertId)) {
				AlertSelectionDialog.this.setMessage(DEFAULT_MESSAGE, IMessageProvider.INFORMATION);
			} else {
