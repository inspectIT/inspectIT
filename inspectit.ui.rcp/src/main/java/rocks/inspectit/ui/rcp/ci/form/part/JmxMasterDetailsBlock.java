...
public class JmxMasterDetailsBlock extends MasterDetailsBlock implements IFormPart, IPartSelectionListener, IPropertyListener {
	...
	public JmxMasterDetailsBlock(FormPage formPage) {
		...
		this.profileData = profile.getProfileData().getIfInstance(JmxDefinitionProfileData.class);
	...
	private IBaseLabelProvider getLabelProvider() {
		return new StyledCellIndexLabelProvider() {
			...
			/**
			 * All domains decoration.
			 */
			private final StyledString allDomains = new StyledString("<all domains>", StyledString.QUALIFIER_STYLER);
			...
			protected StyledString getStyledText(Object element, int index) {
				if (element instanceof JmxBeanSensorAssignment) {
					...
					switch (index) {
					case 0:
						if (StringUtils.isEmpty(jmxAssignment.getDomain())) {
							return allDomains;
						} else {
							return new StyledString(jmxAssignment.getDomain());
						}
					case 1:
