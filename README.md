[![Gitter](https://badges.gitter.im/Join Chat.svg)](https://gitter.im/inspectIT?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge) [![Build Status](http://jenkins.inspectit.rocks/buildStatus/icon?job=inspectIT%20-%20Integration)](http://jenkins.inspectit.rocks/job/inspectIT%20-%20Integration/) [![License](https://img.shields.io/badge/License-AGPLv3--with--exception-GREEN.svg)](https://github.com/inspectIT/inspectIT/blob/master/LICENSE.txt)

# inspectIT APM 
inspectIT (http://inspectit.rocks) is a powerful software product for monitoring and analyzing your software applications. Through various sensors inspectIT captures end-to-end information for every request from the end-user, to the business tier all the way to the backends. inspectIT is based on an application-centric, business-focussed approach, where each technical request is mapped to an application and to a business usecase. With inspectIT you always know about the health of your software and can easily analyze any problems that arise.
To our knowledge we are the first and only professional Open Source APM (Application Performance Management) solution. 

We hope that we together can build an alternative to the (great!) commercial tools available so that it is up the end user to choose his tooling. **Lets make inspectIT even more awesome!**
...
- drill up from an problem within an invocation sequence and find business-related information like the URL that this request was sent to.
- aggregated views for every captured metric, e.g. aggregated SQL overview showing metrics aggregated by SQL query
- advanced analytics by navigating from invocation sequence representation to aggregated views or from aggregated views to invocation sequences (from a slow query in the aggregated view, you can navigate to all invocation sequences where this query is executed)
- All data is recorded. Thus you can diagnose the problems that occurred last week!
- Monitor your hardware metrics like CPU, memory or threads
- Graphical configuration of the captured information
- Transparent integration in the application
...
### Enduser / Development Documentation
