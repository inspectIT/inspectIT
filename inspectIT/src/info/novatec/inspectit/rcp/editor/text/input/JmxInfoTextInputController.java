...
public class JmxInfoTextInputController extends AbstractTextInputController {
	...
	private String[] getAggregatedData(AbstractPlotDataSolver dataSolver) {
		...
		for (DefaultData data : currentData) {
			double avg = dataSolver.valueConvert(((JmxSensorValueData) data).getAverageValue());
			double min = dataSolver.valueConvert(((JmxSensorValueData) data).getMinValue());
			double max = dataSolver.valueConvert(((JmxSensorValueData) data).getMaxValue());
			valueArray[0] += avg;
			if (valueArray[1] > min) {
				valueArray[1] = min;
			}
			...
			count += ((JmxSensorValueData) data).getAggregationCount();
		}
