...
public class JmxPlotController extends AbstractPlotController {
	 ...
	 * Used aggregator.
	 */
	IAggregator<JmxSensorValueData> aggregator = new JmxSensorValueDataAggregator();
	...
	public void preferenceEventFired(PreferenceEvent preferenceEvent) {
		...
		case JMX_PLOTDATASOLVER:
			...
			break;
		case SAMPLINGRATE:
			if (preferenceEvent.getPreferenceMap().containsKey(PreferenceId.SamplingRate.DIVIDER_ID) && preferenceEvent.getPreferenceMap().containsKey(PreferenceId.SamplingRate.SLIDER_ID)) {
				SamplingRate samplingRateIdEnum = (SamplingRate) preferenceEvent.getPreferenceMap().get(PreferenceId.SamplingRate.DIVIDER_ID);
				SamplingRateMode mode = SamplingRateSelecterFactory.selectSamplingRateMode(samplingRateIdEnum);
				
				Sensitivity sensitivity = (Sensitivity) preferenceEvent.getPreferenceMap().get(PreferenceId.SamplingRate.SLIDER_ID);
				
				setSamplingRate(mode, sensitivity);
				break;
			}
		default:
			break;
