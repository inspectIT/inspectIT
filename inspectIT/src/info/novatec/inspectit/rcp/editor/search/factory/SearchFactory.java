...
public final class SearchFactory {
	...
	private static class JmxSearchFinder extends AbstractSearchFinder<JmxSensorValueData> {
		...
		public boolean isSearchCompatible(JmxSensorValueData element, SearchCriteria searchCriteria, RepositoryDefinition repositoryDefinition) {
			...
			if (jmxIdent == null) {
				return false;
			} else if (stringMatches(jmxIdent.getDerivedDomainName(), searchCriteria)) {
				return true;
			} else if (stringMatches(jmxIdent.getDerivedTypeName(), searchCriteria)) {
				return true;
			} else if (stringMatches(jmxIdent.getmBeanAttributeName(), searchCriteria)) {
				return true;
			}
