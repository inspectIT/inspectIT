 ...
 * 
 */
public class EditProfileHanlder extends AbstractHandler implements IHandler {
		...
		if (selected instanceof IProfileProvider) {
			...
			if (Dialog.OK == dialog.open()) {
				...
				try {
					...
					InspectIT.getDefault().getInspectITConfigurationInterfaceManager().profileEdited(updated, true);
				} catch (final Exception e) {
					Display.getCurrent().asyncExec(new Runnable() {
						@Override
						public void run() {
							InspectIT.getDefault().createErrorDialog("Update of the profile state failed.", e, -1);
						}
					});
					return null;
				}
