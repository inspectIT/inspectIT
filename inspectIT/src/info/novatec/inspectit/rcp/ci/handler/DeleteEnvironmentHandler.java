...
public class DeleteEnvironmentHandler extends AbstractHandler implements IHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		confirmDelete.setMessage("Are you sure you want to delete " + size + " the selected environment" + ((size > 1) ? "s" : "") + "?");
		...
		if (confirmed) {
				...
				if (selected instanceof IEnvironmentProvider) {
					...
					try {
						if (repositoryDefinition.getOnlineStatus() != OnlineStatus.OFFLINE) {
							...
							InspectIT.getDefault().getInspectITConfigurationInterfaceManager().environmentDeleted(environment);
						}
					} catch (BusinessException e) {
						failed = true;
						InspectIT.getDefault().log(IStatus.WARNING, "Error deleting environment(s) from the CMR", e);
					}
				}
			...
			if (failed) {
				Display.getCurrent().asyncExec(new Runnable() {
					@Override
					public void run() {
						InspectIT.getDefault().createErrorDialog("Deletion of one " + ((size > 1) ? " or more environments" : " environment") + " failed.", -1);
					}
				});
			}
