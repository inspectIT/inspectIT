	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		if (selected instanceof IEnvironmentProvider) {
			...
			if (Dialog.OK == dialog.open()) {
				...
				try {
					repositoryDefinition.getConfigurationInterfaceService().updateEnvironment(environment);

					InspectIT.getDefault().getInspectITConfigurationInterfaceManager().environmentEdited(environment);
				} catch (final Exception e) {
					Display.getCurrent().asyncExec(new Runnable() {
						@Override
						public void run() {
							InspectIT.getDefault().createErrorDialog("Update of the environment state failed.", e, -1);
						}
					});
					return null;
				}
