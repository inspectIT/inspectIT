...
/**
 * Move {@link IApplicationDefinition} handler.
 *
			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				for (Iterator<?> it = selection.iterator(); it.hasNext();) {
					...
					if (selected instanceof ApplicationLeaf) {
						...
						final IApplicationDefinition application = applicationLeaf.getApplication();
						final CmrRepositoryDefinition repositoryDefinition = applicationLeaf.getCmrRepositoryDefinition();
						...
						if (null == repositoryDefinition) {
							...
							try {
								if (repositoryDefinition.getOnlineStatus() != OnlineStatus.OFFLINE) {
									int oldIndex = applicationLeaf.getIndexInParentList();
									int newIndex = 0;
									...
									if (newIndex < 0 || newIndex >= applicationLeaf.getParentListSize() - 1) {
										return null;
									}
