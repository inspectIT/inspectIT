...
public abstract class AbstractTableBasedManagerView implements CmrRepositoryChangeListener, IRefreshableView {
	...
	public void createControls(Composite parent, boolean multiSelection) {
		...
		Table table = toolkit.createTable(mainForm.getBody(), SWT.V_SCROLL | SWT.H_SCROLL | multiSelectionFlag);
		...
		if (null != workbenchPartSite) {
			workbenchPartSite.setSelectionProvider(selectionProviderAdapter);
		}
		selectionProviderAdapter.setSelection(new StructuredSelection(displayedCmrRepositoryDefinition));
