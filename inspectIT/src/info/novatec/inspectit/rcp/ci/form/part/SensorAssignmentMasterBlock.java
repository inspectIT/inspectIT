...
public class SensorAssignmentMasterBlock extends MasterDetailsBlock implements IFormPart, IPartSelectionListener, ISelectionChangedListener, IPropertyListener {
	...
	/** W I G E T S. */
	private FormPage formPage; // NOCHK
	private IManagedForm managedForm; // NOCHK
	...
	private Composite emptyHintComposite; // NOCHK
	...
	protected void createMasterPart(final IManagedForm managedForm, Composite parent) {
		...
		addButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (null != toAdd) {
					addToInputMap(toAdd);
					// initially every is invalid
					// invalidAssignments.add(toAdd);
	...
	private void createEmptyInputHint() {
		...
		formText.setText("<form><p>Seams that there are no sensor assignments in this profile. Use <b>Add</b> action to start defining instrumentation points.</p></form>", true, false);
	...
	private String getErroMessageShort(AbstractClassSensorAssignment<?> sensorAssignment, List<ValidationControlDecoration<?>> validationDecorations) {
		...
		builder.append((count > 1) ? " fields conatin validation errors)" : " field conatins validation error)");
