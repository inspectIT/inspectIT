			...
			protected IStatus run(IProgressMonitor monitor) {
				...
				if (cmrRepositoryDefinition.getOnlineStatus() != OnlineStatus.OFFLINE) {
					try {
						...
						return new Status(IStatus.ERROR, InspectIT.ID, "Updating of the profile '" + profile.getName() + "' failed due to the exception on the CMR.", e);
					}
				} else {
					return new Status(IStatus.ERROR, InspectIT.ID, "Updating of the profile '" + profile.getName() + "' failed because CMR is currently not online.");
				}
