...
public class TimerSensorAssignmentDetailsPage extends MethodSensorAssignmentDetailsPage {
	...
	/** W I D G E T S. */
	private Button captureContextButton; // NOCHK
	private Button startInvocationButton; // NOCHK
	private Text minDurationText; // NOCHK
	private Button chartingButton; // NOCHK
	private Table captureContextTable; // NOCHK
	private TableViewer captureContextTableViewer; // NOCHK
	private Button addCaptureButton; // NOCHK
	private Button removeCaptureButton; // NOCHK
	 ...
	 */
	public TimerSensorAssignmentDetailsPage(boolean canEdit) {
		super(canEdit);
	}
	...
	public void createContents(Composite parent) {
		...
		// table
		captureContextTable = toolkit.createTable(mainComposite, SWT.SINGLE | SWT.FULL_SELECTION | SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.VIRTUAL);
		...
		addCaptureButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
		...
		removeCaptureButton.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
		...
		captureContextTableViewer.addDoubleClickListener(new IDoubleClickListener() {
			@Override
		...
		// validation
		// method validation
		final ValidationControlDecoration<Text> minDurationValidationDecoration = new ValidationControlDecoration<Text>(minDurationText, this) {
			@Override
		...
		TableWrapData twd = new TableWrapData(TableWrapData.RIGHT);
		twd.colspan = 2;
	...
	protected void updateFromInput() {
		...
		if (null != assignment) {
			if (CollectionUtils.isNotEmpty(assignment.getContextCaptures())) {
				...
				captureContextTable.setEnabled(isCanEdit());
				...
				captureContextButton.setSelection(false);
				captureContextTable.setEnabled(false);
