...
public class SensorAssignmentTableProvider {
	 ...
	 *            reporting
	 */
	public SensorAssignmentTableProvider(Composite parent, ISelectionChangedListener selectionChangedListener) {
		init(parent, selectionChangedListener);
	}
	...
	private void init(Composite parent, ISelectionChangedListener selectionChangedListener) {
		// Table
		Table table = new Table(parent, SWT.MULTI | SWT.FULL_SELECTION | SWT.V_SCROLL | SWT.H_SCROLL | SWT.VIRTUAL);
		...
		return new ImageFixStyledCellIndexLabelProvider() {
			...
			public String getToolTipText(Object element, int index) {
				if (0 == index && element instanceof AbstractClassSensorAssignment) {
					AbstractClassSensorAssignment<?> assignment = (AbstractClassSensorAssignment<?>) element;
					if (assignment.isSuperclass()) {
						return "Super-class instrumentation";
					} else if (assignment.isInterf()) {
	...
	/**
	 * @return Returns comparator for the Table.
	 */
	private ViewerComparator getViewerComparator() {
		return new ViewerComparator() {
			@Override
			public int compare(Viewer viewer, Object e1, Object e2) {
				if (e1 instanceof AbstractClassSensorAssignment && e2 instanceof AbstractClassSensorAssignment) {
					AbstractClassSensorAssignment<?> a1 = (AbstractClassSensorAssignment<?>) e1;
					AbstractClassSensorAssignment<?> a2 = (AbstractClassSensorAssignment<?>) e2;
					if (null != a1.getClassName() && null != a2.getClassName()) {
						return a1.getClassName().compareToIgnoreCase(a2.getClassName());
					} else if (null == a1.getClassName()) {
						return 1;
					} else {
						return -1;
					}
				} else if (e1 instanceof Class<?> && e2 instanceof Class<?>) {
					return ((Class<?>) e1).getName().compareToIgnoreCase(((Class<?>) e2).getName());
				}
				return 0;
			}
		};
