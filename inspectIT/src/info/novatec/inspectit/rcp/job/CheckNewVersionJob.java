...
public class CheckNewVersionJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		try {
			...
			try (InputStream inputStream = entity.getContent()) {
				...
				highestVersionRelease = getHigestVersionFromJson(readed);
			}
		} catch (IOException | InvalidVersionException exception) {
			// give feedback if user triggered it
			if (userTriggered) {
				return new Status(IStatus.ERROR, InspectIT.ID, "Error occurred reading the existing versions from GitHub.", exception);
			} else {
	 ...
	 *             .
	 */
	private VersionRelease getHigestVersionFromJson(JsonArray readed) throws InvalidVersionException {
		Version higest = new Version(0, 0, 0);
		VersionRelease versionRelease = new VersionRelease(higest);
