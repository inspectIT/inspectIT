...
public class BusinessExceptionDialog extends TitleAreaDialog {
	 ...
	 * Dialog title.
	 */
	private static final String DIALOG_TITLE = "Exception occurred";
	...
	public void create() {
		...
		this.setTitleImage(InspectIT.getDefault().getImage(InspectITImages.IMG_WIZBAN_EXCEPTION));
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (null != exception.getActionPerformed()) {
			mainMessage.append("An exception occurred while executing the action <b>" + exception.getActionPerformed() + "</b>. ");
		}
		if (null != exception.getServiceMethodSignature()) {
			mainMessage.append("The exception occurred while invoking the service method <b>" + exception.getServiceMethodSignature() + "</b>. ");
		}
		...
		titleFormText.setColor("headingColor", toolkit.getColors().getColor(IFormColors.TITLE));
		...
		descriptionFormText.setColor("headingColor", toolkit.getColors().getColor(IFormColors.TITLE));
		descriptionFormText.setFont("headingFont", JFaceResources.getFont(JFaceResources.BANNER_FONT));
