...
public class ThrowableDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (throwable instanceof RemoteException) {
			...
			sb.append("The error occurred while invoking the service method <b>" + serviceMethod + "</b>.");
		} else {
			sb.append("The unexpected error occurred in the <b>inspectIT User interface</b>.");
		}
		...
		mainFormText.setColor("header", toolkit.getColors().getColor(IFormColors.TITLE));
		mainFormText.setFont("header", JFaceResources.getFont(JFaceResources.BANNER_FONT));
	...
	private void createDetails(Composite parent) {
		if (!areDetailsDisplayed()) {
			...
			Label label = toolkit.createLabel(detailsComposite, "Exception type:", SWT.WRAP);
			label.setForeground(toolkit.getColors().getColor(IFormColors.TITLE));

			Label exceptionType = toolkit.createLabel(detailsComposite, "", SWT.WRAP);
			exceptionType.setLayoutData(new TableWrapData(TableWrapData.FILL_GRAB));

			label = toolkit.createLabel(detailsComposite, "Exception message:", SWT.WRAP);
			label.setForeground(toolkit.getColors().getColor(IFormColors.TITLE));

			Label exceptionMessage = toolkit.createLabel(detailsComposite, "", SWT.WRAP);
			exceptionMessage.setLayoutData(new TableWrapData(TableWrapData.FILL_GRAB));

			label = toolkit.createLabel(detailsComposite, "Cause:", SWT.WRAP);
			label.setForeground(toolkit.getColors().getColor(IFormColors.TITLE));

			Label exceptionCause = toolkit.createLabel(detailsComposite, "", SWT.WRAP);
			exceptionCause.setLayoutData(new TableWrapData(TableWrapData.FILL_GRAB));
