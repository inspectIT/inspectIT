		...
		public void run() {
			...
			for (IStorageDescriptor storageDescriptor : descriptors) {
				...
				while (descriptorReadSize < descriptorTotalSize) {
					...
					try {
						// execute read
						Future<Void> future = readingChannelManager.read(finalByteBuffer, readPosition, singleReadSize, channelPath, completionRunnable);
						...
						// block until the read is finished, because starting another async read
						// could screw up order of byte buffers and thus screw up the serialization
						try {
							future.get();
						} catch (InterruptedException e) {
							Thread.interrupted();
						} catch (ExecutionException e) {
							log.error("Execution exception waiting for the WriteReadCompletionRunnable in the ReadTask.", e);
						}
					} catch (IOException e) {
