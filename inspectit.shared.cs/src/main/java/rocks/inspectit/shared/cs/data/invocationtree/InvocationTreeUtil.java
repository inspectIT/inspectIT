...
public final class InvocationTreeUtil {
	 ...
	 * <p>
	 * Note: the invocation sequences in the returned list will be only the root invocation
	 * sequences and not their nested child sequences
	 *
	 * @param tree
	...
	public static double calculateSpanExclusiveDuration(InvocationTreeElement element) {
		...
		double childrenDuration = element.getChildren().stream().mapToDouble(InvocationTreeUtil::calculateSpanDuration).sum();
	...
	/**
	 * Returns the duration of the span contained in the given element. If the element is not of
	 * type {@link TreeElementType#SPAN} the sum of all direct child spans are calculated.
	 *
	 * @param element
	 *            the element to calculate the duration
	 * @return the duration of the span or the sum of all direct child spans
	 */
	private static double calculateSpanDuration(InvocationTreeElement element) {
		if (element.isSpan()) {
		...
		double nestedDuration = element.getChildren().stream().mapToDouble(InvocationTreeUtil::calculateSpanDuration).sum();
