...
/**
 * This configuration element indicates that the server name of the HTTP request is to be used as
 * the string value for the {@link StringMatchingExpression}.
 * 
 * @author Marius Oehler
 *
 */
@XmlRootElement(name = "http-server-name")
public class HttpServerNameValueSource extends StringValueSource {
	...
	public String[] getStringValues(InvocationSequenceData invocSequence, ICachedDataService cachedDataService) {
		...
		return new String[] { httpData.getHttpInfo().getServerName() };
	}
}