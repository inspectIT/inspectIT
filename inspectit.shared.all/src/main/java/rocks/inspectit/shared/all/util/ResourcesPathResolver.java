...
public final class ResourcesPathResolver {
	...
	public static File getResourceFile(String resource, File workingDir) throws IOException {
		...
		File mainResourcesFile = new File(workingDir.getAbsolutePath(), "src" + File.separator + "main" + File.separator + "resources" + File.separator + resource);
		if (mainResourcesFile.exists()) {
			return mainResourcesFile;
		}

		// if it does not exist then in the src/main/external-resources
		File extResourcesFile = new File(workingDir.getAbsolutePath(), "src" + File.separator + "main" + File.separator + "external-resources" + File.separator + resource);
