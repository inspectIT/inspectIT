...
import rocks.inspectit.shared.all.tracing.util.RandomUtils;
...
public class SpanIdent implements Sizeable, Serializable {
	...
	/**
	 * Builds a new root span identifier with random.
	 *
	 * @return Builds a new root span identifier with random.
	 */
	public static SpanIdent build() {
		long id = RandomUtils.randomLong();
		return new SpanIdent(id, id, id);
	}

	/**
	 * Creates a new non-root span identifier as child of the given identifier.
	 *
	 * @param parent
	 *            {@link SpanIdent} belonging to the parent span. If <code>null</code> is passed
	 *            then result will be same as calling {@link #build()}.
	 * @return Builds a new non-root span identifier.
	 */
	public static SpanIdent build(SpanIdent parent) {
		if (null != parent) {
			return build(parent.traceId, parent.id);
		} else {
			return build();
		}
	}
	...
	/**
	 * Creates a new non-root span identifier belonging to the given trace ID and having specified
	 * parent ID.
	 *
	 * @param traceId
	 *            Trace span belongs to.
	 * @param parentId
	 *            Parent of the span.
	 * @return Builds a new non-root span identifier.
	 */
	private static SpanIdent build(long traceId, long parentId) {
		long id = RandomUtils.randomLong();
		return new SpanIdent(id, traceId, parentId);
	}
