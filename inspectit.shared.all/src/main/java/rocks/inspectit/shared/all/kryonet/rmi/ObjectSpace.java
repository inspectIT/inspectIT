 ...
 * {@link #register(int, Object) registered} with an ID. The remote end of connections that have
 * been {@link #addConnection(Connection) added} are allowed to
 * {@link #getRemoteObject(Connection, int, Class) access} registered objects.
 * <p>
 * It costs at least 2 bytes more to use remote method invocation than just sending the parameters.
 * If the method has a return value which is not {@link RemoteObject#setNonBlocking(boolean)
...
public class ObjectSpace {
	 ...
	 * {@link #addConnection(Connection) added} to allow the remote end of the connections to access
	 * objects in this ObjectSpace.
	 */
	public ObjectSpace() {
	...
	/**
	 * Registers an object to allow the remote end of the ObjectSpace's connections to access it
	...
	/**
	 * Identical to {@link #getRemoteObject(Connection, int, Class...)} except returns the object
	 * cast to the specified interface type. The returned object still implements
	 * {@link RemoteObject}.
	 */
	...
	/**
	 * Returns a proxy object that implements the specified interfaces. Methods invoked on the proxy
	 * object will be invoked remotely on the object with the specified ID in the ObjectSpace for
	 * the specified connection. If the remote end of the connection has not
	 * {@link #addConnection(Connection) added} the connection to the ObjectSpace, the remote method
	 * invocations will be ignored.
	 * <p>
	 * Methods that return a value will throw {@link TimeoutException} if the response is not
	 * received with the {@link RemoteObject#setResponseTimeout(int) response timeout}.
	 * <p>
	 * If {@link RemoteObject#setNonBlocking(boolean) non-blocking} is false (the default), then
	 * methods that return a value must not be called from the update thread for the connection. An
	 * exception will be thrown if this occurs. Methods with a void return value can be called on
	 * the update thread.
	 * <p>
	...
	static public class InvokeMethod implements FrameworkMessage, KryoSerializable {
		...
		public Object[] args;
		// The top two bytes of the ID indicate if the remote invocation should respond with return
		// values and exceptions,
		// respectively. The rest is a six bit counter. This means up to 63 responses can be stored
	...
	/**
	 * Returns the first object registered with the specified ID in any of the ObjectSpaces the
	 * specified connection belongs to.
	 */
