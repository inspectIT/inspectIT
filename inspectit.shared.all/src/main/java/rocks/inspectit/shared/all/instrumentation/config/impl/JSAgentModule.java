...
public enum JSAgentModule {
	...
	static {
		...
		for (JSAgentModule mod : JSAgentModule.values()) {
			...
			if (Character.isUpperCase(identifier)) {
				throw new RuntimeException("JS Agent Module identifiers have to be lwoer case characters, which is not the case for " + mod.getUiName());
			}
	 ...
	 */
	public static final int EUM_SESSION_MAX_AGE_SECONDS = 60 * 60;
